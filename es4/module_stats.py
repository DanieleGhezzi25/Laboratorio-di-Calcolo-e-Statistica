...
class stats:
    ...
    def hist(self, color='red', xlabel='x', ylabel='y', title='Histogram', label='Histogram'):
        
        bin_intervals = np.linspace(np.min(self.list), np.max(self.list), sturges(len(self.list)))
        
        fig, ax = plt.subplots(1,1)  
        ax.hist(self.list, bins=bin_intervals, label=label, color=color)
