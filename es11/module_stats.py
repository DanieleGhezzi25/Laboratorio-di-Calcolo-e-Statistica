...
class stats:
    ...
    def __init__(self, array = None, txt_name = None, bessel_correction = True):
        ...
        if txt_name is None and array is not None:
            if type(array) != list and type(array) != np.ndarray:
                raise TypeError('Invalid type: insert a list or NumPy array')
            self.list = array
        ...
        if txt_name is not None and array is None:
        ...
        if txt_name is None and array is None:
            self.list = []
            self.mean = 0
            self.variance = 0
            self.std = 0
